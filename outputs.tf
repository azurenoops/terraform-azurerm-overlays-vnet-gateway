...
output "subnet" {
  ...
  value = {
    id   = coalesce(try(azurerm_subnet.vgw[0].id, null), var.existing_gateway_subnet_id)
    name = coalesce(try(azurerm_subnet.vgw[0].name, null), basename(var.existing_gateway_subnet_id))
  }
}
